...
int main()
{
	...
	double SLR_float = 0.0000000001;
	PlatformModel.InitialiseSeaLevel(SLR_float);
	int num_SLR = 3;
	...
	double Resistance = 0.01;                               // 0.001 >> soft-rock, 0.1 >> hard-rock
	double WeatheringRate = 0.001;                          // Z x 1e^4 [mm/year] (maximum)
	PlatformModel.InitialiseGeology(CliffHeight, CliffFailureDepth, Resistance, WeatheringRate);

    //wave erodibility
    double WaveAttenuConst = 0.05;                           // 0.01 >> Large wave erosion efficacy, 1 >> Small wave erosion efficacy
